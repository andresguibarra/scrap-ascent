...
func _ready() -> void:
	...
	# Connect to state machine signal for debugging (deferred to ensure state machine is ready)
...
func _connect_state_machine_signals() -> void:
	...
	if not state_machine.state_changed.is_connected(_on_state_changed):
		...
		print("Enemy: Connected to state machine signals")
	else:
		print("Enemy: Already connected to state machine signals")
...
func _on_state_changed() -> void:
	...
	# Update current_state based on the active state machine state
	...
	if state_name.begins_with("AI"):
		...
		print("Enemy: Unknown state name: ", state_name)
		return
...
func _process(_delta: float) -> void:
	...
	# State machine will handle all processing
	# Keep animation updates here since they're called by states
	pass
...
func _update_wall_sliding() -> void:
	var tier_skills = skills.get(tier, [Skill.MOVE, Skill.JUMP])
	if Skill.WALL_CLIMB not in tier_skills or current_state != Enemy.State.CONTROLLED:
		if is_wall_sliding:
...
func posses():
	current_state = Enemy.State.CONTROLLED
	_update_visual_state()
	_activate_possession_light()
	# Let the state machine transition to the appropriate controlled state
	if state_machine and state_machine.has_method("transition_to"):
		if is_on_floor():
			state_machine.transition_to("ControlledIdleState")
		else:
			state_machine.transition_to("ControlledFallState")
...
func _apply_horizontal_movement(direction_x: float) -> void:
	...
	if current_state == Enemy.State.CONTROLLED and is_on_floor() and direction_x != 0:
		var input_direction := sign(direction_x) as int
		
		# Check if direction changed
		if input_direction != last_input_direction:
			direction_change_timer = 0.035  # 75ms threshold
...
func _release_control() -> void:
	...
	if chip_destroyed:
		current_state = Enemy.State.INERT
	else:
		current_state = Enemy.State.AI
	...
	# Let the state machine transition to the appropriate state
	if state_machine and state_machine.has_method("transition_to"):
		if chip_destroyed:
			if is_on_floor():
				state_machine.transition_to("InertIdleState")
			else:
				state_machine.transition_to("InertFallState")
		else:
			if is_on_floor():
				state_machine.transition_to("AIIdleState")
			else:
				state_machine.transition_to("AIFallState")
...
func destroy_chip() -> void:
	...
	if current_state != Enemy.State.CONTROLLED:
		...
		_play_destroy_sound()
		# Let the state machine transition to the appropriate inert state
		if state_machine and state_machine.has_method("transition_to"):
			if is_on_floor():
				state_machine.transition_to("InertIdleState")
			else:
...
func _update_sprite_flip() -> void:
	...
	elif current_state == Enemy.State.AI:
	...
	if current_flip != should_flip_right:
		...
		if eyes_sprite and current_state == Enemy.State.CONTROLLED:
